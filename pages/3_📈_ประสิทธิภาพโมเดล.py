...
with col1:
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar=False, xticklabels=['ปกติ', 'ขาดน้ำ'], yticklabels=['ปกติ', 'ขาดน้ำ'])
...
roc_auc = auc(fpr, tpr)
fig_roc = px.area(x=fpr, y=tpr, title=f'ROC Curve (AUC = {roc_auc:.4f})', labels=dict(x='False Positive Rate', y='True Positive Rate'), width=800, height=500)
...
try:
    ...
    linear_model.fit(scaler.transform(X_full), df['dehydration_status'])
    importance = pd.DataFrame({'Feature': X_full.columns, 'Importance': abs(linear_model.coef_[0])}).sort_values('Importance', ascending=False)
    fig_imp = px.bar(importance, x='Importance', y='Feature', orientation='h', color='Importance', color_continuous_scale='Viridis')
    st.plotly_chart(fig_imp, use_container_width=True)
except:
    st.warning("ไม่สามารถคำนวณ Feature Importance ได้")